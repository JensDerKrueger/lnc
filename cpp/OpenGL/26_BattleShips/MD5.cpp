...
uint32_t MD5::rotateLeft(uint32_t x, int n) const {
  return (x << n) | (x >> (32-n));
}
...
void MD5::byteToUInt(uint32_t* Output, const uint8_t* Input, uint32_t nLength, int& error) {
  //entry invariants
...
void MD5::uIntToByte(uint8_t* Output, const uint32_t* Input, uint32_t nLength, int& error) {
  //entry invariants
