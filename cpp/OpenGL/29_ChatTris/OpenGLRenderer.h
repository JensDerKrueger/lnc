...
public:
	...
	void dropAnimation(const std::array<Vec2i,4>& source, const Vec3& sourceColor,
                     const std::array<Vec2i,4>& target, const std::vector<uint32_t>& clearedRows) override;
	bool isAnimating() const override;
  void setGameOver(bool gameOver, uint32_t score) override;
	
  virtual void updateHighscore(const std::vector<std::pair<std::string, uint32_t>>& highscore) override;
...
private:	
  ...
  PrecomputedParticleSystem scoreParticleSystem;
	Vec3 viewerPos;
  ...
  std::vector<std::pair<std::string, uint32_t>> topScores;
