...
void Scene::renderWorld(float t0, const Mat4& m, const Mat4& v, const Mat4& p, const Dimensions& dim) {
	...
	progNormalMap.setUniform(mvpLocationNormalMap, mBall*m*v*p);
	progNormalMap.setUniform(mLocationNormalMap, mBall*m);
	progNormalMap.setUniform(mitLocationNormalMap, Mat4::inverse(mBall*m), true);
	...
	const Mat4 mBackWall{Mat4::translation(0.0f,0.0f,-2.0f)*m};
	...
	// ************* fresnel frame
	if (showFresnelFrame) fresnelBall->render(mBall*m*v,p);
