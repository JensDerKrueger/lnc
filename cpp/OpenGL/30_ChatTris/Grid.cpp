...
void Grid::render(const std::array<Vec2i,4>& tetrominoPos, const Vec3& currentColor, 
				  ...
				  const std::array<Vec2i,4>& targetTetrominoPos, float time) const {
	if (!renderer->isAnimating()) {
		std::vector<Vec3> colorData(width*height);
		std::transform(data.begin(), data.end(), colorData.begin(), 
						[](int8_t v) -> Vec3 { return v == -1 ? Vec3(-1.0f,-1.0f,-1.0f) : colors[size_t(v)]; });
		renderer->setObstacles(colorData);
	}
	renderer->render(tetrominoPos, currentColor, nextTetrominoPos, nextColor, targetTetrominoPos, time);
}
