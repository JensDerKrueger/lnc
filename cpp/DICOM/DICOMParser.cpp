...
#include "DICOMParser.h"
  ...
  #include <stdio.h>
  #include <sstream>
  #define DICOM_DBG(...) printf(__VA_ARGS__)
#else
  #define DICOM_DBG(...)
#endif
...
bool StacksSmaller ( DICOMStackInfo* pElem1, DICOMStackInfo* pElem2 ) {
  return pElem1->m_iSeries < pElem1->m_iSeries;
}
...
void DICOMParser::GetDirInfo(string  strDirectory) {
  ...
  for (size_t i = 0;i<files.size();i++) {
    //cout << "DICOMParser::GetDirInfo Looking for DICOM data in file "<< files[i] << endl;
    DICOMFileInfo info;
  ...
  //cout << "DICOMParser::GetDirInfo "<<fileInfos.size()<<" files in candidate list." << endl;
