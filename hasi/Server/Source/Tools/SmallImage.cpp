...
namespace IVDA
{
  ...
  std::string SmallImage::Convert(const std::string& filename, const std::string& newExt) {
    std::stringstream ss;
    ss << "convert \"" << filename << "\" \"" << IVDA::SysTools::ChangeExt(filename,newExt) << "\"";
    ...
    return IVDA::SysTools::ChangeExt(filename,newExt);
  }
#endif
}
