...
class Tetris:
	...
	def __startGame(self):
		...
		self.score = 0 # BUG: MISSING
		self.level = 0 # BUG: MISSING
	...
	def fullDrop(self):
		...
		while self.__validateTransform(self.rotation, position):
			self.position = position
			position = [self.position[0],self.position[1]+1] # BUG:  position[1] += 1
	...
	def render(self):
		...
		tetrominoPos = self.__computeGridPosition(self.current, self.rotation , self.position)
		for p in tetrominoPos:
			spriteData += [[p[0],p[1],self.current,1]]

		# create sprite for next tetromino preview
		tetrominoPos = self.__computeGridPosition(self.next, 0, [2,0])
		for p in tetrominoPos:
			spriteData += [[p[0],p[1],self.next,0.2]]
			
		self.grid.render(spriteData)
		##### MISSING
	...
	def __updateScore(self, rowCount):
		...
		if self.totalRows % 10 == 0:  # BUG: MISSING
			self.level += 1 # BUG: MISSING
